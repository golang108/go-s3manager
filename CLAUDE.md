...
**Frontend** (`web/`): Server-rendered HTML using three Go templates (`layout.html.tmpl`, `buckets.html.tmpl`, `bucket.html.tmpl`) with Materialize CSS and jQuery. No JS framework. Static assets and templates are embedded into the binary via `//go:embed`.
